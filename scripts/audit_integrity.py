...
def main() -> int:
    ...
    unaud = 0
    total = 0
    roots = [str(ROOT / "runs*")] + [str(Path(r).expanduser()) for r in (args.runs_root or [])]
    paths: list[str] = []
    for r in roots:
        paths += glob.glob(str(Path(r) / "*/*/"))
    for res in sorted(set(paths)):
        ws = Path(res)
        ...
        try:
            ...
            rel = ws  # a run root outside the repo (a valid ablation)
        score = None
        rj = ws / "result.json"
        if rj.exists():
            try:
    ...
    if total == 0:
        ...
        print("VACUOUS: no runs found under runs*/ — this audit checked NOTHING.\n"
