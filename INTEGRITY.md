...
### A different blind spot: agents routed around broken tools

One bundled planner failed on every invocation across five experimental boards. Agents did
not stop or surface the defect. They wrote replacement searches and continued solving the
games, so score and ledger audits remained green. The parenthetical phrase "or agent-written
searches" in our own methods table had been the bug report in plain sight.

This was not reward hacking, but it exposed a distinct evaluator failure: auditing outcomes
does not prove that the supplied system works when an autonomous agent can self-heal around
broken infrastructure. `tests/test_tool_smoke.py` now executes every workspace tool and
checks the failure class directly.
...
* **Public-set replay, not held-out verification.** ARC's server re-executed the reported
  action traces, but the underlying 25 games are public and the result is not a semi-private
...
* **Reruns vary.** Frontier models make thousands of autonomous judgment calls,
  and provider behavior can drift.
