...
\title{Kepler: 100\% on ARC-AGI-3\\
at About One Quarter of Tycho's Estimated Cost}
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\linewidth}{@{}p{3.2cm}r>{\raggedleft\arraybackslash}p{2.8cm}>{\raggedleft\arraybackslash}p{2.6cm}X@{}}
    ...
    \textbf{Kepler + Opus 5} & 100.00 & 858.0M (97.37\% cache reads) & \textbf{\$777.72} (list-equiv.) & prepared; publication pending \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\bigskip
\paragraph{Appendices.} Full per-game tables for all boards, the guard-deadlock and dead-code forensic timelines, the sp80 search inventory, the lf52 replay transcripts, and the audit flag taxonomy are provided in the repository as tables and write-ups. The raw append-only ledgers they were derived from belong to the companion trace corpus, which is prepared but not yet published (\S\ref{sec:repro}); until it is, the appendices can be read but not independently recomputed from source data.
