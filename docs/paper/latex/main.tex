...
\textbf{GPT-6 Astra.} ARC Prize reports 62.71 on the semi-private set under its
provider-neutral Standard interface and 99.95 with a Provider Adapter that
preserves opaque reasoning state and compacts long interactions
~\citep{arcprize2026astra}. Astra's trajectories construct compact symbolic
world models without Kepler's external executable-model protocol. This is
stronger generalization evidence than our public-set board and further rules
out treating world-model construction or human-relative action efficiency as
unique Kepler contributions. Our narrower contribution is an externally
checkable contract joining predictions, actions, workspace state, selection,
and replay, plus an audit record that changed our own claims.
